document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkAdminAccess() {
        const savedKey = localStorage.getItem('admin_gate_key');
        gateInput.value = '';
        gateErrorEl.style.display = 'none';
        gateInput.parentElement.parentElement.classList.remove('has-error');

        if (!savedKey) {
            // First time setup: prompt user to create their gateway password
            gateBadge.textContent = 'INITIAL SETUP';
            gateTitle.textContent = 'Create Key';
            gateSubtitle.textContent = 'Set access password for the admin portal';
            gateInputLabel.textContent = 'Choose New Access Key';
            gateSubmitBtn.textContent = 'Save & Enter';
        } else {
            // Verification check
    ...
    function submitGateKey() {
        ...
        if (!savedKey) {
            // First time registration: save the key
            localStorage.setItem('admin_gate_key', enteredKey);
            closeGateModal();
            openAdminPanel();
        } else {
    ...
    adminSearchInput.addEventListener('input', loadAdminTable);
